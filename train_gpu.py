...
import time
from model import Gomoku, GomokuNetV3, get_valid_action_torch, load_model_if_exists, is_adjacent_to_piece_torch
...
def get_device_config():
    """
    自动检测并返回可用的设备（CUDA -> DirectML -> CPU）。
    """
    if torch.cuda.is_available():
        device = torch.device("cuda")
        print(f"[初始化] 主设备: {device} | 支持CUDA: True")
        return device, "cuda"
    
    if torch_directml.is_available():
        try:
            device = torch_directml.device(1)
            print(f"[初始化] 主设备: {device} | 支持DirectML: True")
            return device, "dml"
        except RuntimeError as e:
            print(f"[警告] 无法创建DirectML设备：{e}。退回到CPU。")
    
    device = torch.device("cpu")
    print(f"[初始化] 未找到任何GPU设备，使用CPU。")
    return device, "cpu"
...
def env_worker(env_id, board_size, win_condition, model_queue, experience_queue, total_cells, device):
    env = Gomoku(board_size, win_condition, device)
    ...
    while True:
        ...
        while not done and total_steps < STEPS_PER_ENV:
            state_tensor = env.get_state_representation_torch().unsqueeze(0).to(device)
            ...
            action = get_valid_action_torch(logits, board_flat, board_size, epsilon=current_epsilon)
            
            current_player = env.current_player
            next_player, done, reward = env.step_torch(action)
            episode_experience.append((state_tensor.cpu().detach(), action.cpu().detach(), reward.cpu().detach(), current_player))
            total_steps += 1
...
def update_model_batch(model, optimizer, policy_criterion, value_criterion, batch, device):
    ...
    actions = torch.tensor([a.item() for a in actions], device=device)
    rewards = torch.tensor([r.item() for r in rewards], device=device, dtype=torch.float32)

    logits, values = model(states)
    num_classes = logits.size(1)
    actions_one_hot = torch.zeros(actions.size(0), num_classes, device=device).scatter_(1, actions.unsqueeze(1), 1)

    log_probs = F.log_softmax(logits, dim=1)
    policy_loss = -(log_probs * actions_one_hot).sum(dim=1) * rewards
    policy_loss = policy_loss.mean()
    
    value_loss = value_criterion(values.squeeze(-1), rewards)
...
def train():
    ...
    main_device, device_type = get_device_config()
    ...
    processes = []
    total_envs = PARALLEL_ENVS

    for env_id in range(PARALLEL_ENVS):
        p = Process(
            target=env_worker, 
            args=(
                f"{device_type}-{env_id}", config.BOARD_SIZE, config.WIN_CONDITION,
                model_queue, experience_queue, total_cells, main_device
            ),
            daemon=True
        )
        p.start()
    ...
    print(f"[并行初始化] 启动 {total_envs} 个{device_type.upper()}游戏环境")
    ...
    try:
        while total_episodes < config.MAX_EPISODES:
            ...
            if current_time - last_print_time >= 10:
                ...
                print(f"[速度统计] 过去 {time_diff:.1f} 秒完成 {episodes_diff} 局")
                print(f" 总体平均速度: {speed:.1f} 局/秒")
                print(" ------------------------")
        ...
        if total_time > 0:
            ...
            print(f"设备贡献: {device_type.upper()}: {total_episodes} 局 (100.0%)")
