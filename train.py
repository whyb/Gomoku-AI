...
# 设备特定配置参数
CPU_PARALLEL_ENVS = 6    # CPU环境数量
GPU_PARALLEL_ENVS = 4    # GPU环境数量
STEPS_PER_ENV = 50       # 每个环境每轮生成的步数
...
def env_worker(env_id, board_size, win_condition, model_queue, experience_queue, 
    ...
    while True:
        ...
        while not done and total_steps < STEPS_PER_ENV:
            ...
            # 转换为CPU进行后续处理
            valid_actions = list(zip(valid_logits.cpu().numpy().flatten(), valid_indices.cpu().numpy()))
            ...
            episode_experience.append((state.cpu().detach(), action, reward, current_player))
            total_steps += 1
...
def train():
    ...
    for env_id in range(CPU_PARALLEL_ENVS):
        p = Process(
            target=env_worker,
            args=(f"cpu-{env_id}", config.BOARD_SIZE, config.WIN_CONDITION, 
    ...
    #        p = Process(
    #            target=env_worker,
    #            args=(f"gpu-{env_id}", config.BOARD_SIZE, config.WIN_CONDITION, 
    ...
    print(f"[并行初始化] 启动 {total_envs} 个并行游戏环境 (CPU: {CPU_PARALLEL_ENVS}, GPU: {GPU_PARALLEL_ENVS if use_cuda else 0})")
    ...
    try:
        while total_episodes < config.MAX_EPISODES:
            ...
            if len(replay_buffer1) >= batch_size and len(replay_buffer2) >= batch_size:
                ...
                loss1, _ = update_model_batch(model1, optimizer1, criterion, batch1, main_device)
                loss2, _ = update_model_batch(model2, optimizer2, criterion, batch2, main_device)
                update_steps += 1
